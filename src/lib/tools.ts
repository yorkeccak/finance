...
export const financeTools = {
  ...
  createChart: tool({
    ...
    inputSchema: z.object({
      title: z.string().describe('Chart title (e.g., "Apple vs Microsoft Stock Performance")'),
      type: z.enum(['line', 'bar', 'area']).describe('Chart type - use "line" for time series data like stock prices'),
      xAxisLabel: z.string().describe('X-axis label (e.g., "Date", "Quarter", "Year")'),
      yAxisLabel: z.string().describe('Y-axis label (e.g., "Price ($)", "Revenue (Millions)", "Percentage (%)")'),
      dataSeries: z.array(z.object({
        name: z.string().describe('Series name - include company/ticker for stocks (e.g., "Apple (AAPL)", "Tesla Revenue")'),
        data: z.array(z.object({
          x: z.union([z.string(), z.number()]).describe('X-axis value - use date strings like "2024-01-01" for time series'),
          y: z.number().describe('Y-axis numeric value - stock price, revenue, percentage, etc.')
        })).describe('Array of data points with x (date/label) and y (value) properties')
      })).describe('REQUIRED: Array of data series - each series has name and data array with x,y objects'),
      description: z.string().optional().describe('Optional description explaining what the chart shows')
    }),
    execute: async ({ title, type, xAxisLabel, yAxisLabel, dataSeries, description }) => {
      ...
      const chartData = {
        ...
        metadata: {
          ...
          totalDataPoints: dataSeries.reduce((sum, series) => sum + series.data.length, 0),
          dateRange: dataSeries.length > 0 && dataSeries[0].data.length > 0 
            ? {
      ...
      console.log('[Chart Creation] Chart data size:', JSON.stringify(chartData).length, 'bytes');
  ...
  codeExecution: tool({
    ...
    inputSchema: z.object({
      code: z.string().describe('Python code to execute - MUST include print() statements to display results. Use descriptive output formatting with labels, units, and proper number formatting.'),
      description: z.string().optional().describe('Brief description of what the calculation or analysis does (e.g., "Calculate future value with compound interest", "Analyze portfolio risk metrics")'),
    }),
    execute: async ({ code, description }) => {
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || (typeof window === 'undefined' ? '' : window.location.origin) || 'http://localhost:3000'}/api/python`, {
          method: 'POST',
          headers: {
        ...
        if (!response.ok) {
          return `❌ **API Error**: ${result.error || 'Failed to execute Python code'}`;
        }
...
⏱️ **Execution Time**: ${result.executionTime}ms`;
        ...
        return `❌ **Network Error**: Failed to connect to Python execution service. ${error instanceof Error ? error.message : 'Unknown error occurred'}`;
      }
    },
  }),
  
  financialSearch: tool({
    description: 'Search for comprehensive financial data including real-time market data, earnings reports, SEC filings, regulatory updates, and financial news using Valyu DeepSearch API',
    inputSchema: z.object({
      query: z.string().describe('Financial search query (e.g., "Apple latest quarterly earnings", "Bitcoin price trends", "Tesla SEC filings")'),
      dataType: z.enum(['auto', 'market_data', 'earnings', 'sec_filings', 'news', 'regulatory']).optional().describe('Type of financial data to focus on'),
      maxResults: z.number().min(1).max(20).optional().default(10).describe('Maximum number of results to return. This is not number of daya/hours of stock data, for example 1 yr of stock data for 1 company is 1 result'),
    }),
    execute: async ({ query, dataType, maxResults }) => {
      try {
        ...
        console.log('[Financial Search] Full API Response:', JSON.stringify(response, null, 2));
        ...
        console.log('[Financial Search] Formatted response size:', JSON.stringify(formattedResponse).length, 'bytes');
  ...
  webSearch: tool({
    description: 'Search the web for general information on any topic using Valyu DeepSearch API with access to both proprietary sources and web content',
    inputSchema: z.object({
      query: z.string().describe('Search query for any topic (e.g., "benefits of renewable energy", "latest AI developments", "climate change solutions")'),
      maxResults: z.number().min(1).max(20).optional().default(5).describe('Maximum number of results to return'),
      relevanceThreshold: z.number().min(0).max(1).optional().default(0.5).describe('Minimum relevance score (0-1) for results'),
      maxPrice: z.number().min(0).optional().default(20.0).describe('Maximum cost in dollars for the search'),
    }),
    execute: async ({ query, maxResults, relevanceThreshold, maxPrice }) => {
      try {
        ...
        // Log the full API response for debugging
        console.log('[Web Search] Full API Response:', JSON.stringify(response, null, 2));
        ...
        console.log('[Web Search] Formatted response size:', JSON.stringify(formattedResponse).length, 'bytes');
        ...
        if (error instanceof Error) {
          ...
          if (error.message.includes('price') || error.message.includes('cost')) {
